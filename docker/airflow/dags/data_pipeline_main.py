...
def run_dbt_models(**context):
    ...
    try:
        ...
        result = subprocess.run(
            ['dbt', 'run', '--target', dbt_config['target']],
            cwd=dbt_config['project_path'],
            env=env,
            capture_output=True,
            text=True,
        ...
        test_result = subprocess.run(
            ['dbt', 'test', '--target', dbt_config['target']],
            cwd=dbt_config['project_path'],
            env=env,
            capture_output=True,
            text=True,
...
def update_clickhouse_realtime(**context):
    ...
    if transactions:
        ...
        try:
            ...
            clickhouse_user = Variable.get("clickhouse_user", default_var=clickhouse_config['user'])
            clickhouse_password = Variable.get("clickhouse_password", default_var=clickhouse_config['password'])
