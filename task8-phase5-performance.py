...
# Configuration
POSTGRES_HOST = "postgresql.data-ingestion.svc.cluster.local"
POSTGRES_DB = "ecommerce"
POSTGRES_USER = "postgres"
POSTGRES_PASSWORD = "postgres"
TARGET_RATE = 1000  # events per second
DURATION = 60  # seconds
LOG_DIR = os.environ.get('LOG_DIR', './task8-logs')
...
def log(message):
    ...
    with open(f"{LOG_DIR}/phase5.log", "a") as f:
...
def benchmark_performance():
    ...
    log(f"Starting performance benchmark: {TARGET_RATE} events/sec for {DURATION} seconds")
    ...
    def insert_batch():
        ...
        try:
            ...
            values = [(f"perf_test_data_{i}_{source_timestamp}", source_timestamp) 
                     for i in range(batch_size)]
            
            cur.executemany(
                "INSERT INTO perf_test (data, source_timestamp) VALUES (%s, %s)", 
                values
            )
            conn.commit()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        while time.time() - start_time < DURATION:
            ...
            batches_per_second = TARGET_RATE / 100
